    ...
    <div class="seat-box">
      <div class="tick-box">
        <ul class="tick clearfix" :style="{width: width + 'px'}">
          <li ref="userchair" v-for="(item, index) in userChairData" :key="index"><span class="c c1"></span><i class="r r1"></i>{{item.row}}排{{item.col}}座<span class="c c2"></span><i class="r r2"></i></li>
        </ul>
      </div>
      ...
      <div class="chair-flag clearfix">
        ...
        <div>
          <div class="c-box clearfix">
            <i class="c-bg fl c-bg1"></i><span class="fl buyed">已售</span>
          </div>
        </div>

        <div>
          <div class="c-box clearfix m-box">
            <i class="c-bg fl c-bg2"></i><span class="fl buyed">已选</span>
          </div>
        </div>

        <div>
          <div class="c-box clearfix fr">
            <i class="c-bg fl c-bg3"></i><span class="fl buyed">可选</span>
          </div>
        </div>
    ...
    methods: {
      ...
      selectChair(item) {
        ...
        if (item.type == '') {
          ...
          //提交actions
          this.$store.dispatch('buyMovieTickModule/addUserChairData', item)
            .then(() => {
              ...
              this.$store.commit('buyMovieTickModule/modifyUlWidth', (width + marginRight) *  this.userChairData.length);
            })
          ...
          this.$store.dispatch('buyMovieTickModule/deleteUserChairData', item)
            .then(() => {
              let firstLi = this.$refs.userchair[0];
  ...
  .buy-movie-tick{
    ...
    .screen{
      ...
      .screen-arc1{
        ...
        height: 200px;
        border-radius: 50%;
        border: 3px solid rgba(255, 243, 199, .6);
        position: absolute;
        top: 18px;
        left: -100px;
      }
    ...
    .buy-tick{
      ...
      .pay{
        width: 80px;
        height: 34px;
        background-image: linear-gradient(to bottom, #f886cc, rgb(253, 109, 222));
        color: #fff;
        font-size: 13px;
        text-align: center;
        line-height: 34px;
        border-radius: 10px;
        border-bottom: 2px solid #eb48ad;
        letter-spacing: 2px;
        margin-top: 10px;
      }
    }
